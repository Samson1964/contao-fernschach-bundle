	...
	(
		...
		(
			...
			(
				...
				'button_callback'     => array('tl_fernschach_turniere_spieler', 'toggleIcon')
			),
			'show' => array
			(
...
class tl_fernschach_turniere_spieler extends \Backend
{
	...
	public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_fernschach_turniere_spieler::published', 'alexf'))
		{
			return '';
		}
	...
	public function toggleVisibility($intId, $blnPublished)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_fernschach_turniere_spieler::published', 'alexf'))
		{
			$this->log('Not enough permissions to show/hide record ID "'.$intId.'"', 'tl_fernschach_turniere_spieler toggleVisibility', TL_ERROR);
			$this->redirect('contao/main.php?act=error');
		}

		$this->createInitialVersion('tl_fernschach_turniere_spieler', $intId);

		// Trigger the save_callback
		if (is_array($GLOBALS['TL_DCA']['tl_fernschach_turniere_spieler']['fields']['published']['save_callback']))
		{
			foreach ($GLOBALS['TL_DCA']['tl_fernschach_turniere_spieler']['fields']['published']['save_callback'] as $callback)
			{
		...
		// Update the database
		$this->Database->prepare("UPDATE tl_fernschach_turniere_spieler SET tstamp=". time() .", published='" . ($blnPublished ? '' : '1') . "' WHERE id=?")
		               ->execute($intId);
		$this->createNewVersion('tl_fernschach_turniere_spieler', $intId);
	}
