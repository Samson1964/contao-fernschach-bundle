...
(
	...
	(
		...
		(
			...
			array('tl_fernschach_spieler_konto_nenngeld', 'getInfo'),
	...
	(
		...
		(
			...
			'showColumns'             => true,
			'label_callback'          => array('tl_fernschach_spieler_konto_nenngeld', 'getLabel')
		),
		...
		(
			...
			'moveHaupt' => array
			(
				'label'               => &$GLOBALS['TL_LANG']['tl_fernschach_spieler_konto_nenngeld']['moveHaupt'],
				'href'                => 'key=moveBuchung&source=n&target=h',
				'icon'                => 'bundles/contaofernschach/images/konto_haupt_move.png',
				'attributes'          => 'onclick="if(!confirm(\'' . ($GLOBALS['TL_LANG']['tl_fernschach_spieler_konto_nenngeld']['moveHauptConfirm'] ?? null) . '\'))return false;Backend.getScrollOffset()"',
			),
			'moveBeitrag' => array
			(
				'label'               => &$GLOBALS['TL_LANG']['tl_fernschach_spieler_konto_nenngeld']['moveBeitrag'],
				'href'                => 'key=moveBuchung&source=n&target=b',
				'icon'                => 'bundles/contaofernschach/images/konto_beitrag_move.png',
				'attributes'          => 'onclick="if(!confirm(\'' . ($GLOBALS['TL_LANG']['tl_fernschach_spieler_konto_nenngeld']['moveBeitragConfirm'] ?? null) . '\'))return false;Backend.getScrollOffset()"',
			),
			'markiertIcon' => array
			(
	...
	(
		...
		'kategorie' => array(
			'label'                   => &$GLOBALS['TL_LANG']['tl_fernschach_spieler_konto_nenngeld']['kategorie'],
			'exclude'                 => true,
			'sorting'                 => true,
			'filter'                  => true,
			'inputType'               => 'select',
			'options'                 => &$GLOBALS['TL_LANG']['tl_fernschach_spieler_konto_nenngeld']['kategorie_options'],
			'eval'                    => array
			(
				'tl_class'            => 'w50 clr',
				'multiple'            => false,
		...
		'saldo' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_fernschach_spieler_konto_nenngeld']['saldo'],
		),
	)
);
...
class tl_fernschach_spieler_konto_nenngeld extends Backend
{
	...
	public function getInfo(DataContainer $dc)
	{
		...
		$objRecord = \Schachbulle\ContaoFernschachBundle\Models\Nenngeldkonto::findByPk($dc->id);
		if($objRecord === null)
		{
			return;
		}
