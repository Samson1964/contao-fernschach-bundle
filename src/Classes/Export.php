		...
		if(Input::post('FORM_SUBMIT') == 'tl_fernschach_exportexcel')
		{
			...
			foreach($arrExport as $item)
			{
				$spreadsheet->getActiveSheet()
				            ->getStyle('AV'.$zeile)->getNumberFormat()->setFormatCode('#,##0.00_-"€"'); // vorher setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE)
				$spreadsheet->getActiveSheet()
				            ->getStyle('AW'.$zeile)->getNumberFormat()->setFormatCode('#,##0.00_-"€"'); // vorher setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE)
				$spreadsheet->getActiveSheet()
				            ->getStyle('AX'.$zeile)->getNumberFormat()->setFormatCode('#,##0.00_-"€"'); // vorher setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE)
				$spreadsheet->getActiveSheet()
